...
public class MyCustomLanguageImpl implements Language {
	...
	public String format(
		...
		if (GetterUtil.getBoolean(PropsUtil.get(PropsKeys.TRANSLATIONS_DISABLED))) {
			return pattern;
		}
	...
	public String get(Locale locale, String key, String defaultValue) {
		...
		String value = LanguageResourcesClp.getMessage(locale, key);
		...
		while ((value == null) || value.equals(defaultValue)) {
			if ((key.length() > 0) &&
				...
				if (pos != -1) {
					...
					value = LanguageResourcesClp.getMessage(locale, key);
	...
	public void updateCookie(
		...
		languageIdCookie.setMaxAge(CookieKeysClp.MAX_AGE);
		...
		CookieKeysClp.addCookie(request, response, languageIdCookie);
	...
	private MyCustomLanguageImpl(long companyId) {
		...
		if (companyId != CompanyConstants.SYSTEM) {
			try {
				...
				localesArray = PropsUtil.getArray(PropsKeys.LOCALES);
			}
		}
		...
		String[] localesBetaArray = PropsUtil.getArray(PropsKeys.LOCALES_BETA);
	...
	private String _get(
		...
		if (portletConfig != null) {
			...
			if (value != null) {
				value = LanguageResourcesClp.fixValue(value);
			}
		}
	...
	private static final Log _log = LogFactoryUtil.getLog(MyCustomLanguageImpl.class);
